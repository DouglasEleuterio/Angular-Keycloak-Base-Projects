...
@Component({
  selector: 'app-companhia-aerea-list',
  templateUrl: './list.component.html',
  styleUrls: ['./list.component.scss']
})
export class ListComponent extends PaginatorComponent {
  tableData: CompanhiaAerea[] = [];

  constructor(
    public service: CompanhiaAereaService,
    ...
    this.breadcrumbService.setItems([
      ...
      {
        label: 'companhia-aerea.title.page'
      }
    ]);
  ...
  updateStatus(id: string, ativo: boolean): void {
    this.baseController.active(
      ...
      ativo,
      this.service,
      ativo ? 'companhia-aerea.message.inactived' : 'companhia-aerea.message.actived',
      () => this.fetch()
    );
  }
  ...
  remove(id: string): void {
    ...
    this.service
      ...
      .subscribe({
        next: () => {
          this.alertService.defaultSuccess(this.translateService.instant('companhia-aerea.message.deleted_success'.toUpperCase()));
          this.fetch();
        },
  ...
  get exporterConfig(): ExporterTypeConfig {
    const baseColumns = ['id;Código', 'nome;Nome', 'situacao;Ativo;Sim,Não'];
