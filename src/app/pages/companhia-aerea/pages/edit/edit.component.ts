...
@Component({
  selector: 'app-companhia-aerea-edit',
  template: '<app-companhia-aerea-form [isNew]="false" #form></app-companhia-aerea-form>'
})
export class EditComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.baseController.load(this.route, this.service, this.menuBack, (entity: CompanhiaAerea) => {
      this.id = entity.id;
      this.onLoad(entity);
    });
  }
  ...
  onLoad(entity: CompanhiaAerea): void {
    if (entity == null) {
      this.router
        .navigate(this.menuBack.routerLink)
        .then(() => this.alertService.defaultError(this.translateService.instant('companhia-aerea.message.not_found'.toUpperCase())));
    } else {
      this.entity = entity;
  ...
  onSubmit(entity: CompanhiaAerea, formGroup: FormGroup): void {
    ...
    entity.id = this.id;
    this.baseController.saveRedirect(formGroup, this.service.update(entity), 'companhia-aerea.message.updated', this.menuBack, () =>
