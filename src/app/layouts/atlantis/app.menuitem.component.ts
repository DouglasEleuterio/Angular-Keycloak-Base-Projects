...
import { AppMenuItem } from 'src/app/domain/menu/app-menu.model';
...
@Component({
  ...
  template: `
    <ng-container>
      <div *ngIf="item.isLabel" class="layout-menuitem-root-text">{{ item.label.toUpperCase() | translate }}</div>
      <a
        [attr.href]="item.url"
      ...
      >
        ...
        <span class="layout-menuitem-text">{{ item.label.toUpperCase() | translate }}</span>
      ...
      <ul
        *ngIf="(item.items && active) || (item.items && app.isStatic() && (root || active))"
        [@children]="
  ...
  `,
  host: {
    '[class.layout-root-menuitem]': 'false',
